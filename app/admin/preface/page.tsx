...
export default function PrefacePage() {
  ...
  useEffect(() => { fetchPrefaces(); }, [page, search, verificationStatus, sort]);

  const fetchPrefaces = async () => {
    try {
      setLoading(true);
      ...
      const res = await fetch(`/api/preface?${params}`);
      const data = await res.json();
      setPrefaces(data.prefaces);
      setPagination(data.pagination);
    } catch (error) {
      console.error('Failed to fetch prefaces:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`/api/preface/${id}`, { method: 'DELETE' });
      if (res.ok) fetchPrefaces();
    } catch (error) {
      console.error('Failed to delete preface:', error);
    }
    setDeleteId(null);
  };
  ...
  const handleVerify = async (id: string) => {
    try {
      const res = await fetch(`/api/preface/${id}/verify`, { method: 'POST' });
      if (res.ok) fetchPrefaces();
    } catch (error) {
      console.error('Failed to verify preface:', error);
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Preface</h1>
          <p className="text-gray-500 mt-1">Manage Quran preface entries</p>
        </div>
        <Link
          href="/admin/preface/new"
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer select-none hover:bg-gray-100" onClick={() => handleSort('suraId')}>
                  <span className="inline-flex items-center gap-1">Sura ID <SortIcon field="suraId" /></span>
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer select-none hover:bg-gray-100" onClick={() => handleSort('prefaceSubTitle')}>
                  <span className="inline-flex items-center gap-1">Subtitle <SortIcon field="prefaceSubTitle" /></span>
                </th>
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-500">No prefaces found</td></tr>
              ) : (
                prefaces.map((preface) => (
                  <tr key={preface._id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm font-medium text-gray-900">{preface.customId}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{preface.suraId}</td>
                    <td className="px-4 py-3">
                      <p className="text-sm font-medium text-gray-900 truncate max-w-xs">{preface.prefaceSubTitle}</p>
                    </td>
                    <td className="px-4 py-3">
                      {preface.isVerified ? (
                    ...
                    <td className="px-4 py-3">
                      <p className="text-sm text-gray-900">{preface.createdBy}</p>
                      <p className="text-xs text-gray-500">{preface.createdByRole}</p>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        {user?.role === 'verifier' && !preface.isVerified && (
                          <button onClick={() => handleVerify(preface._id)} className="p-2 text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="Verify">
                            <FiCheckCircle className="w-4 h-4" />
                          </button>
                        )}
                        <Link href={`/admin/preface/${preface._id}/edit`} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                          <FiEdit2 className="w-4 h-4" />
                        </Link>
                        <button onClick={() => setDeleteId(preface._id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full mx-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">Delete Preface</h3>
            <p className="text-gray-500 mb-4">Are you sure you want to delete this preface? This action cannot be undone.</p>
