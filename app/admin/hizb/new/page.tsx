...
export default function NewHizbPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/hizb', {
        method: 'POST',
      ...
      if (!res.ok) { setError(data.error || 'Failed to create hizb record'); return; }
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Add Hizb Record</h1>
          <p className="text-gray-500 mt-1">Create a new hizb division entry</p>
        </div>
      </div>
