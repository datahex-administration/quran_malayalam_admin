...
export default function EditHizbPage() {
  ...
  useEffect(() => { fetchHizb(); }, [id]);

  const fetchHizb = async () => {
    try {
      const res = await fetch(`/api/hizb/${id}`);
      if (!res.ok) { router.push('/admin/hizb'); return; }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setError(data.error || 'Failed to update hizb record'); return; }
      router.push('/admin/hizb');
    } catch {
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        <Link href="/admin/hizb" className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><FiArrowLeft className="w-5 h-5" /></Link>
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Edit Hizb Record</h1>
          <p className="text-gray-500 mt-1">Update hizb division details</p>
        </div>
      </div>
      <div className="bg-white rounded-xl shadow-sm p-6">
        <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
          ...
          <div className="flex gap-4">
            ...
            <Link href="/admin/hizb" className="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">Cancel</Link>
          </div>
        </form>
      </div>
    </div>
