...
export default function HizbPage() {
  const [hizbList, setHizbList] = useState<HizbRecord[]>([]);
  ...
  useEffect(() => { fetchHizb(); }, [page, search, verificationStatus, sort]);
  ...
  const fetchHizb = async () => {
    try {
      ...
      const res = await fetch(`/api/hizb?${params}`);
      const data = await res.json();
      setHizbList(data.hizbList);
      setPagination(data.pagination);
    } catch (error) {
      console.error('Failed to fetch hizb records:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`/api/hizb/${id}`, { method: 'DELETE' });
      if (res.ok) fetchHizb();
    } catch (error) {
      console.error('Failed to delete hizb record:', error);
    }
    setDeleteId(null);
  };
  ...
  const handleVerify = async (id: string) => {
    try {
      const res = await fetch(`/api/hizb/${id}/verify`, { method: 'POST' });
      if (res.ok) fetchHizb();
    } catch (error) {
      console.error('Failed to verify hizb record:', error);
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Hizb</h1>
          <p className="text-gray-500 mt-1">Manage Hizb division records</p>
        </div>
        <Link
          href="/admin/hizb/new"
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500">No hizb records found</td></tr>
              ) : (
                hizbList.map((hizb) => (
                  <tr key={hizb._id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm font-medium text-gray-900">{hizb.customId}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{hizb.chapterNo}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{hizb.verseNo}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{hizb.position}</td>
                    ...
                    <td className="px-4 py-3">
                      <p className="text-sm text-gray-900">{hizb.createdBy}</p>
                      <p className="text-xs text-gray-500">{hizb.createdByRole}</p>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        {user?.role === 'verifier' && !hizb.isVerified && (
                          <button onClick={() => handleVerify(hizb._id)} className="p-2 text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="Verify">
                            <FiCheckCircle className="w-4 h-4" />
                          </button>
                        )}
                        <Link href={`/admin/hizb/${hizb._id}/edit`} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                          <FiEdit2 className="w-4 h-4" />
                        </Link>
                        <button onClick={() => setDeleteId(hizb._id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full mx-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">Delete Hizb Record</h3>
