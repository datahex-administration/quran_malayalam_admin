...
export default function ArabicAyahsPage() {
  const [ayahs, setAyahs] = useState<ArabicAyah[]>([]);
  ...
  useEffect(() => { fetchAyahs(); }, [page, search, verificationStatus, sort]);
  ...
  const fetchAyahs = async () => {
    try {
      ...
      const res = await fetch(`/api/arabic-ayahs?${params}`);
      const data = await res.json();
      setAyahs(data.ayahs);
      setPagination(data.pagination);
    } catch (error) {
      console.error('Failed to fetch Arabic ayahs:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`/api/arabic-ayahs/${id}`, { method: 'DELETE' });
      if (res.ok) fetchAyahs();
    } catch (error) {
      console.error('Failed to delete Arabic ayah:', error);
    }
    setDeleteId(null);
  };
  ...
  const handleVerify = async (id: string) => {
    try {
      const res = await fetch(`/api/arabic-ayahs/${id}/verify`, { method: 'POST' });
      if (res.ok) fetchAyahs();
    } catch (error) {
      console.error('Failed to verify Arabic ayah:', error);
    }
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Arabic Quran Ayahs</h1>
          <p className="text-gray-500 mt-1">Manage Arabic ayah data</p>
        </div>
        <Link
          href="/admin/arabic-ayahs/new"
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search by chapter or Arabic text..."
              value={search}
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Verse Range</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500">No Arabic ayahs found</td></tr>
              ) : (
                ayahs.map((ayah) => (
                  <tr key={ayah._id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm font-medium text-gray-900">{ayah.customId}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{ayah.chapterNo}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{ayah.verseFrom} – {ayah.verseTo}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{ayah.position}</td>
                    ...
                    <td className="px-4 py-3">
                      <p className="text-sm text-gray-900">{ayah.createdBy}</p>
                      <p className="text-xs text-gray-500">{ayah.createdByRole}</p>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        {user?.role === 'verifier' && !ayah.isVerified && (
                          <button onClick={() => handleVerify(ayah._id)} className="p-2 text-green-600 hover:bg-green-50 rounded-lg transition-colors" title="Verify">
                            <FiCheckCircle className="w-4 h-4" />
                          </button>
                        )}
                        <Link href={`/admin/arabic-ayahs/${ayah._id}/edit`} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                          <FiEdit2 className="w-4 h-4" />
                        </Link>
                        <button onClick={() => setDeleteId(ayah._id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
      ...
      {deleteId && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full mx-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-2">Delete Arabic Ayah</h3>
