...
export default function EditArabicAyahPage() {
  ...
  useEffect(() => { fetchAyah(); }, [id]);

  const fetchAyah = async () => {
    try {
      const res = await fetch(`/api/arabic-ayahs/${id}`);
      if (!res.ok) { router.push('/admin/arabic-ayahs'); return; }
      const data = await res.json();
      setFormData({
        customId: data.customId.toString(),
        chapterNo: data.chapterNo.toString(),
        verseFrom: data.verseFrom.toString(),
        verseTo: data.verseTo.toString(),
        dataArabic: data.dataArabic,
        position: data.position.toString(),
      });
    } catch { router.push('/admin/arabic-ayahs'); }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setError(data.error || 'Failed to update Arabic ayah'); return; }
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Edit Arabic Ayah</h1>
          <p className="text-gray-500 mt-1">Update Arabic ayah details</p>
        </div>
      </div>
