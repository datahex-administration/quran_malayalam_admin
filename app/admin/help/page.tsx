...
import { FiPlus, FiEdit2, FiTrash2, FiCheckCircle, FiHelpCircle } from 'react-icons/fi';
...
export default function HelpPage() {
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`/api/help/${id}`, { method: 'DELETE' });
      if (res.ok) {
        fetchItems();
      }
    } catch (error) {
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">Manage help articles and FAQs</p>
        </div>
        <Link
          href="/admin/help/new"
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm p-8 text-center text-gray-500">
            No help articles found. Add your first help article.
          </div>
        ) : (
          items.map((item) => (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full mx-4">
            ...
            <p className="text-gray-500 mb-4">
              Are you sure you want to delete this help article?
            </p>
