...
export default function NewChapterDescriptionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/chapter-descriptions', {
        method: 'POST',
      ...
      if (!res.ok) { setError(data.error || 'Failed to create chapter description'); return; }
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Add Chapter Description</h1>
          <p className="text-gray-500 mt-1">Create a new chapter description entry</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div>
              ...
              <input type="number" value={formData.customId} onChange={(e) => setFormData({ ...formData, customId: e.target.value })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" placeholder="Unique ID" required />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" value={formData.position} onChange={(e) => setFormData({ ...formData, position: e.target.value })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent" placeholder="Order position" />
            </div>
          </div>
